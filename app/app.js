...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/window.js");
/******/ })
/************************************************************************/
/******/ ({
...
window.showdown = showdown__WEBPACK_IMPORTED_MODULE_6___default.a; // --------------------- --------------------- ---------------------
// application state
...
electron__WEBPACK_IMPORTED_MODULE_5__["ipcRenderer"].on('reload-window', (event, reloadContents) => {
...
function show(html, target) {
  let w = window.open("", target, "toolbar=no,scrollbars=yes,resizable=yes,width=800,height=500"); // close the old window so we can open with focus
  ...
  link.type = "text/css";
  link.rel = "stylesheet";
  const cssPath = path__WEBPACK_IMPORTED_MODULE_1___default.a.resolve(__dirname, './main.css');
  console.log(cssPath);
  link.href = cssPath;
...
function loadFile(path) {
  fs__WEBPACK_IMPORTED_MODULE_0___default.a.readFile(path, function (err, data) {
    if (err) {
...
function saveFile(event, url) {
  ...
  if (fileName === template) {
    ...
    electron__WEBPACK_IMPORTED_MODULE_5__["remote"].dialog.showSaveDialog(electron__WEBPACK_IMPORTED_MODULE_5__["remote"].getCurrentWindow(), options, fileNames => {
    ...
    fs__WEBPACK_IMPORTED_MODULE_0___default.a.writeFile(fileName, fileContent, function (err) {
      if (err) {
        return alert(err);
      }
